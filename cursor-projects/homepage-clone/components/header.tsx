...
const Header = memo(function Header() {
  return (
    <header className="w-full py-3 px-4 md:py-4 md:px-16 flex items-center justify-between sticky top-0 z-10 backdrop-blur-sm bg-opacity-90 dark:bg-opacity-90 bg-[#e6f0ff] dark:bg-[#1a1a2e] transition-all duration-300">
        ...
        <nav className="hidden md:flex items-center space-x-8" role="navigation" aria-label="Main navigation">
          {[
            ...
            <Link
              key={index}
              href={item.href}
              className="text-sm text-gray-800 dark:text-gray-200 hover:text-[#4d8eff] dark:hover:text-[#6d9eff] transition-colors duration-300 uppercase tracking-wide font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 rounded-sm"
            >
              {item.name}
            </Link>
          ))}
